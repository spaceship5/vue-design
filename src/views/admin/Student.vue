...
export default {
  methods: {
    ...
    async handleDelete(index, row) {
      let res = await this.$http.delete("/admin/student/" + row.id);
      console.log(res);
    ...
    async submit() {
      let res = await this.$http.post("/admin/student", this.submitForm);
      if (res != null) {
        this.centerDialogVisible = false;
        this.$router.go(0);
  ...
  async created() {
    let res = await this.$http.get("/admin/students");
    console.log(res);
